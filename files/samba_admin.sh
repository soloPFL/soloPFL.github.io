...
#   - Generating an fstab entry to mount a share on a client machine (saved to a text file)
#   - Displaying a manual that explains all options and what they do
#
# Sensible defaults are used when settings are not specified (e.g. shares are browsable,
# with secure file and directory masks, and default permissions).
#
...
configure_global_settings() {
    ...
    # Update (or inject) the [global] section using awk.
...
add_share() {
    ...
    # Set secure default permissions: owner and group have access.
    chmod 0770 "$dirpath"

    # Append the share block with defaults: browsable, writable, guest disabled,
    # and secure file creation masks.
    {
        echo ""
        echo "[$sharename]"
        echo "   path = $dirpath"
...
##############################
view_logs() {
    # Attempt to show the tail of the primary Samba log.
...
2. Add New Share:
   ...
   - The share is created with default settings (browsable, writable, guest access disabled,
     with secure file (0660) and directory (0770) masks). Optionally, restrict access to specified Samba users.
...
6. Add Samba User:
   ...
   - If the Unix system user does not exist, you can choose to create it.
